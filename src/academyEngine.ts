...
function normalizeNodes(nodes: Partial<MasteryNode>[], path: string): MasteryNode[] {
  return nodes.slice(0, 6).map((node, index) => {
    ...
    return {
      ...
      concept: node.concept ?? buildConcept(path, frameTitle, index),
...
function generateFallbackNodes(path: string): MasteryNode[] {
  return nodeFrames.map(([frameTitle, objective], index) => ({
    ...
    objective,
    concept: buildConcept(path, frameTitle, index),
...
function buildConcept(path: string, frameTitle: string, index: number) {
